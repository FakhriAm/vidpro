<?php defined('BASEPATH') OR exit('No direct script access allowed');
class My_request_model extends CI_Model {
 
    var $table = 'content_provider';

    public function __construct(){
        parent::__construct();
        $this->load->database();
    }
 
    
 
    public function get_list_company(){
		$this->db->select('id,content');
        $this->db->from('video_source');
        $this->db->where('active',1);
        $result = $this->db->get()->result();
		$lists = array();
        foreach ($result as $row) $lists[$row->id] = $row->content;	
        return $lists;
    }
    
    public function get_datatables_myrequest(){
        $this->_get_datatables_myrequest_query();
        if($_POST['length'] != -1)
        $this->db->limit($_POST['length'], $_POST['start']);
        return $this->db->get()->result();
    }


    private function _get_datatables_myrequest_query(){
        $this->db->select('request_transaction');
        $this->db->from($this->table);   
        $this->db->where('active',1);
        $this->db->sort('request_date');
        // $i = 0;
        // foreach ($this->column_search as $item){
        //     if($_POST['search']['value']){
        //         if($i===0) {
        //             $this->db->group_start();
        //             $this->db->like($item, $_POST['search']['value']);
        //         } else $this->db->or_like($item, $_POST['search']['value']);
        //         if(count($this->column_search) - 1 == $i) $this->db->group_end();
        //     }
        //     $i++;
        // }
        // if(isset($_POST['order'])) $this->db->order_by($this->column_order[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
        // else if(isset($this->order)){
        //     $order = $this->order;
        //     $this->db->order_by(key($order), $order[key($order)]);
        // }
    }
}