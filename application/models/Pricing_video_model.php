...
class Pricing_video_model extends CI_Model {
    ...
    private function _query(){
        $this->db->select('video_meta.*,CONCAT(users.first_name," ",users.last_name) as uploader');
    ...
    public function get_list_video_type(){
    ...
    public function get_list_video_price(){
    	$this->db->select('id,content');
        $this->db->from('video_price');
        $this->db->where('active',1);
        $result = $this->db->get()->result();
		$lists = array();
        foreach ($result as $row) $lists[$row->id] = 'Rp '.number_format($row->content);
        return $lists;
    }
